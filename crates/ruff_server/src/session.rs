...
use lsp_types::{PositionEncodingKind, ServerCapabilities, Url};
use notify::Watcher;
...
impl Session {
    pub(crate) fn new(
        ...
        Ok(Self {
            position_encoding: server_capabilities
                ...
                .unwrap_or_default(),
            lsp_settings: types::Settings::default(),
    ...
    pub(crate) fn set_encoding(
        ...
        self.position_encoding = encoding_kind
            .clone()
            .try_into()
            .map_err(|()| anyhow!("Unexpected encoding value {}", encoding_kind.as_str()))?;
        Ok(())
    }
